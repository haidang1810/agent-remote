...
export function registerDockerContainerStartTool(db: Database.Database): void {
  upsertTool(db, {
  ...
  addToolDefinition({
    ...
    async handler(args) {
      try {
        ...
        if (msg.includes('already started') || msg.includes('304')) {
          return errorResult(`Container "${args.container}" is already running`);
        }
        ...
        return errorResult(`docker_container_start error: ${msg}`);
      }
    },
  });
}
