...
export function registerPm2RestartTool(db: Database.Database): void {
  upsertTool(db, {
    name: 'pm2_restart',
    category: 'pm2',
    description: 'Restart a PM2 managed application by name',
    risk_level: 'high',
  });
  ...
  addToolDefinition({
    ...
    description: 'Restart a PM2 managed application by name',
    schema: {
      appName: z.string().describe('PM2 application name to restart'),
    },
    async handler(args) {
      ...
      try {
        await safeExec(`pm2 restart ${appName}`);
